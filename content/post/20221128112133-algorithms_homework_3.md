...
date = 2022-12-14T16:13:00+02:00
tags = ["math"]
draft = false
+++
...
we consider the [recurrence relation]({{< relref "20221105001640-recursive_function.md#recurrence-relation" >}}) \\(T(n)=2T\left(\frac{n}{2}\right) + C\\): <br/>
...
note that although it doesnt matter, \\(C\_1\\) differs a from \\(C\\) as \\(C\_1=T(1)\\) is the constant time it takes for the base case calls of the recursion <br/>
...
find the time complexity of the algorithm using [recursion tree]({{< relref "20221105001640-recursive_function.md#recursion-tree" >}})s, find the number of nodes in each level and the number of levels <br/>

<div class="note">

im almost sure there is something wrong here but im not sure how to improve this solution <br/>

</div>

<div class="answer">

we propose the following [recurrence relation]({{< relref "20221105001640-recursive_function.md#recurrence-relation" >}}): <br/>
\\[
  T(n) = 3T\left(\frac{3}{4}n\right) + C
\\] <br/>
and build a recursion tree from it: <br/>

![](/ox-hugo/hzown4g.svg) <br/>
let \\(x\\) denote the height of the tree, we know \\(\left(\frac{3}{4}\right)^xn < 10\\) because \\(n < 10\\) is the [base case]({{< relref "20221105001640-recursive_function.md#base-case" >}}) <br/>
the number of nodes in each level \\(y\\) is \\(3^{y-1}\\) <br/>
according to the pattern in the tree: <br/>
\\[
T(n) = \sum\_{y=0}^{x-1} 3^yC
\\] <br/>
we might have to check the time complexity for each value in \\(T(1),\dots,T(9)\\) assigned as the base case only if they do play a role in the total time complexity <br/>
the time complexity is the sum of all the nodes in the tree: <br/>
...
\begin{gather\*}
  ...
  T(n) = \sum\_{y=0}^{x-1} 3^yC = \left(\frac{3^x}{2} - \frac{1}{2}\right)C = \left(\frac{3^\frac{\ln9 - \ln n}{\ln\left(\frac{3}{4}\right)}}{2} - \frac{1}{2}\right)C\\\\
...
it confused me for a good while that we have \\(-\ln n\\) and i thought that the expression would converge as \\(n\\) goes to infinity but then i realized that \\(\ln\frac34\\) is negative and it would cancel the minus sign so the time would actually diverge <br/>

</div>

we can notice that the constant \\(9\\) played no part in the equation so the result would be the same for all constants 1 to 9 <br/>
so the time complexity is \\(\Theta\left(3^n\right)\\) <br/>
the height of the tree is dependant on the initial value of \\(n\\) but i approximate it to be \\(\frac{\ln9 - \ln n}{\ln\left(\frac{3}{4}\right)}\\) <br/>
...
{{< figure src="/ox-hugo/dZUUjjV.svg" >}} <br/>
