...
\author{mahmood}
\date{\today}
\title{disjoint events}
\hypersetup{
 pdfauthor={mahmood},
 pdftitle={disjoint events},
 pdfkeywords={},
 pdfsubject={},
...
\begin{definition}
events A and B are \textbf{disjoint} if they cannot occur at the same time, \(P(A \cap B) = 0\)\\[0pt]
\begin{lemma}
given A,B are disjoint events, \[ P(A \cup B) = P(A) + P(B) \], and vice versa\\[0pt]
\begin{proof}
generally, \(P(A \cup B) = P(A)+P(B)-P(A \cap B)\) so \(P(A \cap B)=\varnothing\) iff \(P(A \cup B) = P(A)+P(B)\)\\[0pt]
\end{proof}
\end{lemma}
\end{definition}
\end{document}